...
**Provider-specific pricing and retired models** — Each provider card shows a short note (`.provider-pricing-note`). **Retired/deprecated models are excluded from all sections** of the app. Exclusion is implemented in two places: (1) **State:** `filterRetiredModels()` in `src/app.js` runs on every `setData()`, so in-memory data (Overview pricing grid, and any list built from `getData()`) never includes retired models. (2) **Lists:** `getAllModels()` and `getUnifiedCalcModels()` in `src/calculator.js` skip retired models using `isRetired(providerKey, name)` from `src/utils/retiredModels.js`. Retired detection helpers (`isRetiredGeminiModel`, `isRetiredOpenAIModel`, `isRetiredAnthropicModel`, `isRetiredMistralModel`) live in `src/utils/retiredModels.js` and are used by app.js and re-exported from `render.js`. **Sections where retired models do not appear:** Overview (current pricing grid), **Models** (comparison table), **Value Analysis** (cost vs performance chart), **Calculators** (model dropdown and compare), **Benchmarks** (benchmark table), **Recommend** (recommendation results). Excluded examples: Gemini 1.0, `gemini-pro`; OpenAI `gpt-4-0314`, `gpt-3.5-turbo-instruct`, `babbage-002`; Anthropic Claude 3 Opus, 3.5 Haiku, 3.7 Sonnet; Mistral `mistral-large`, `mistral-small`, `open-mistral-nemo`.
...
**Retired models (excluded in all sections)** — Retired/deprecated models are never shown. Logic lives in **`src/utils/retiredModels.js`** (`isRetiredGeminiModel`, `isRetiredOpenAIModel`, `isRetiredAnthropicModel`, `isRetiredMistralModel`, and `isRetired(providerKey, name)`). **App state:** `src/app.js` calls `filterRetiredModels()` before every `setData()`, so Overview and any code using `getData()` only see non-retired models. **Model lists:** `src/calculator.js` uses `isRetired()` inside `getAllModels()` and `getUnifiedCalcModels()`, so **Models**, **Value Analysis**, **Calculators**, **Benchmarks**, and **Recommend** all exclude retired models even if they received unfiltered data. No section shows retired models.
